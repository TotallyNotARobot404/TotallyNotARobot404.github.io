// SQLInjection.cpp : This file contains the 'main' function. Program execution begins and ends there.
// This is artifact 1 for my CS-499 Class with SNHU by Tammie Pease

#include <algorithm>
#include <iostream>
#include <locale>
#include <tuple>
#include <vector>

#include "sqlite3.h"

// set up tuple and strings
typedef std::tuple<std::string, std::string, std::string> user_record;
const std::string str_where = " where ";
//prepare prepared statements
typedef struct sqlite3_stmt sqlite3_stmt;


static int callback(void* possible_vector, int argc, char** argv, char** azColName)
{
    if (possible_vector == NULL)
    { // no vector passed in, so just display the results
        for (int i = 0; i < argc; i++)
        {
            std::cout << azColName[i] << " = " << (argv[i] ? argv[i] : "NULL") << std::endl;
        }
        std::cout << std::endl;
    }
    else
    {
        std::vector< user_record >* rows =
            static_cast<std::vector< user_record > *>(possible_vector);

        rows->push_back(std::make_tuple(argv[0], argv[1], argv[2]));
    }
    return 0;
}

// Initializes the database
bool initialize_database(sqlite3* db)
{
    char* error_message = NULL;
    std::string sql = "CREATE TABLE USERS(" \
        "ID INT PRIMARY KEY     NOT NULL," \
        "NAME           TEXT    NOT NULL," \
        "PASSWORD       TEXT    NOT NULL);";

    int result = sqlite3_exec(db, sql.c_str(), callback, NULL, &error_message);
    if (result != SQLITE_OK)
    {
        std::cout << "Failed to create USERS table. ERROR = " << error_message << std::endl;
        sqlite3_free(error_message);
        return false;
    }
    std::cout << "USERS table created." << std::endl;

    // insert some dummy data
    sql = "INSERT INTO USERS (ID, NAME, PASSWORD)" \
        "VALUES (1, 'Fred', 'Flinstone');" \
        "INSERT INTO USERS (ID, NAME, PASSWORD)" \
        "VALUES (2, 'Barney', 'Rubble');" \
        "INSERT INTO USERS (ID, NAME, PASSWORD)" \
        "VALUES (3, 'Wilma', 'Flinstone');" \
        "INSERT INTO USERS (ID, NAME, PASSWORD)" \
        "VALUES (4, 'Betty', 'Rubble');";

    //test if not working
    result = sqlite3_exec(db, sql.c_str(), callback, NULL, &error_message);
    if (result != SQLITE_OK)
    {
        std::cout << "Data failed to insert to USERS table. ERROR = " << error_message << std::endl;
        sqlite3_free(error_message);
        return false;
    }

    return true;
}

//sanitize query before doing it
void clean_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    //add " around to help sanitize statements
    std::string sqlTemp = "\"" + sql + "\"";
    std::cout << sqlTemp << std::endl;
}

//runs the query to test injection
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    //run cleaner before testing for injection
    clean_query(db, sql, records);
    records.clear();
    //refurbished code using ' or which is more likely an injection than not
    //updated code to not specify why it has stopped outside of 'strange occurance'
    if (sql.find("' or") != std::string::npos) {
        std::cout << "A strange occurance within the query appeared. Please retry." << std::endl;
        return false;
    }

    //error catch
    char* error_message;
    if (sqlite3_exec(db, sql.c_str(), callback, &records, &error_message) != SQLITE_OK)
    {
        std::cout << "Data failed to be queried from USERS table. ERROR = " << error_message << std::endl;
        sqlite3_free(error_message);
        return false;
    }

    return true;
}

// runs the injection code to test if injectable
bool run_query_injection(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    std::string injectedSQL(sql);
    std::string localCopy(sql);

    // we work on the local copy because of the const
    std::transform(localCopy.begin(), localCopy.end(), localCopy.begin(), ::tolower);
    if (localCopy.find_last_of(str_where) >= 0)
    { // this sql has a where clause
        if (localCopy.back() == ';')
        { // smart SQL developer terminated with a semicolon - we can fix that!
            injectedSQL.pop_back();
        }

        //random test to confirm works for all
        switch (rand() % 4)
        {
        case 1:
            injectedSQL.append(" or 2=2;");
            break;
        case 2:
            injectedSQL.append(" or 'hi'='hi';");
            break;
        case 3:
            injectedSQL.append(" or 'hack'='hack';");
            break;
        case 0:
        default:
            injectedSQL.append(" or 1=1;");
            break;
        }
    }

    return run_query(db, injectedSQL, records);
}

// show results of injection attempt
void dump_results(const std::string& sql, const std::vector< user_record >& records)
{
    std::cout << std::endl << "SQL: " << sql << " ==> " << records.size() << " records found." << std::endl;

    for (auto record : records)
    {
        std::cout << "User: " << std::get<1>(record) << " [UID=" << std::get<0>(record) << " PWD=" << std::get<2>(record) << "]" << std::endl;
    }
}

// runs the query
void run_queries(sqlite3* db)
{
    char* error_message = NULL;
    std::vector< user_record > records;
    sqlite3_stmt* st;

    // query all
    std::string sql = "SELECT * from USERS";

    if (!run_query(db, sql, records)) return;
    dump_results(sql, records);

    //query selection
    std::string name = "";
    std::cout << "Insert Name:\n";
    std::cin >> name;

    //prepared statement
    //sql = "SELECT ID, NAME, PASSWORD FROM USERS WHERE NAME='Fred'";
    sql = "SELECT ID, NAME, PASSWORD FROM USERS WHERE NAME = '" + name + "'";
    sqlite3_prepare(db, sql.c_str(), -1, &st, NULL);
    sqlite3_bind_text(st, 1, name.c_str(), name.length(), SQLITE_TRANSIENT);
    sqlite3_step(st);
    sqlite3_finalize(st);

    if (!run_query(db, sql, records)) return;
    dump_results(sql, records);

    //  run query 1 with injection 5 times
    for (auto i = 0; i < 5; ++i)
    {
        if (!run_query_injection(db, sql, records)) continue;
        dump_results(sql, records);
    }

}

//insert new user into USER SQL
void insert_new(sqlite3* db)
{
    //initialize variables
    sqlite3_stmt* st;
    int id = 0;
    std::string name = "";
    std::string password = "";
    char* error_message = NULL;
    std::vector< user_record > records;

    //Get info from user
    std::cout << "Insert ID:\n";
    std::cin >> id;
    std::cout << "Insert Name:\n";
    std::cin >> name;
    std::cout << "Insert Password:\n";
    std::cin >> password;

    //prepared statement
    //insert into the system
    std::string sql = "INSERT INTO USERS (ID, NAME, PASSWORD) VALUES (?, ?, ?);";

    sqlite3_prepare(db, sql.c_str(), -1, &st, NULL);
    sqlite3_bind_int(st, 1, id);
    sqlite3_bind_text(st, 2, name.c_str(), name.length(), SQLITE_TRANSIENT);
    sqlite3_bind_text(st, 3, password.c_str(), password.length(), SQLITE_TRANSIENT);
    sqlite3_step(st);
    sqlite3_finalize(st);

    //check if it worked
    sql = "SELECT ID, NAME, PASSWORD FROM USERS WHERE NAME= '" + name + "'";
    if (!run_query(db, sql, records)) return;
    dump_results(sql, records);
}



// main with prepared statements
int main()
{
    // initialize random seed:
    srand(time(nullptr));
    //setting up the input of the system
    std::string selection = "";
    std::vector< user_record > records;
    std::string sql = "";
    // the database handle
    sqlite3* db = NULL;
    char* error_message = NULL;

    int return_code = 0;
    std::cout << "SQL Injection Example" << std::endl;

    // open the database connection
    int result = sqlite3_open(":memory:", &db);

    //test if connection available
    if (result != SQLITE_OK)
    {
        std::cout << "Failed to connect to the database and terminating. ERROR=" << sqlite3_errmsg(db) << std::endl;
        return -1;
    }

    //signify pass
    std::cout << "Connected to the database." << std::endl;

    // initialize our database
    if (!initialize_database(db))
    {
        std::cout << "Database Initialization Failed. Terminating." << std::endl;
        return_code = -1;
    }

    //selection for everything
    while (selection != "exit")
    {
        //set up insert or run queries
        std::cout << "Select" << std::endl;
        std::cout << "insert to add new data" << std::endl;
        std::cout << "query to look for showing the full table and looking up one individual or" << std::endl;
        std::cout << "exit to quit" << std::endl;
        std::cin >> selection;

        if (selection == "query")
        {
            run_queries(db);
        }
        else if (selection == "insert")
        {
            insert_new(db);
        }
        else if (selection == "exit")
        {
            return(0);
        }
    }

    // close the connection if opened
    if (db != NULL)
    {
        sqlite3_close(db);
    }

    return return_code;
}

// Run program: Ctrl + F5 or Debug > Start Without Debugging menu
// Debug program: F5 or Debug > Start Debugging menu
